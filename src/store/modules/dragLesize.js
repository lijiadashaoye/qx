...
const enum_ = {
    qrcode:{height:130,width:130},
    title:{height:40},
    presetText: { height: 40,width:355 },
    textComponent: { height: 40,width:355 },
    presetImg: { height: 135,width:240 },
    imgComponent: { height: 135,width:240 },
    // presetMedia: { height: 200 },
    // mediaComponent: { height: 200 }
}
const state = {
    'pages_modulesRects': { modules: [] },//页面数据
    'rects': [
        // {
        //     'width': 200,
        //     'height': 150,
...
const mutations = {
    ...
    [SETCOM_ATTR](state, { attr, key, value }) {
...
const actions = {
    setComAttr({ commit, state }, { key, attr, value }) {

        commit(types.SETCOM_ATTR, { key, attr, value })
    },
    getpage_modulesRects({ commit, state }, { url }) {
        // [GETPAGE_MODULESRECTS](state,newModulesRects){
        //     state.pages_modulesRects=newModulesRects
        // }
    ...
    resetFontcolor_rect({ commit, state }, { index, color }) {
        commit(types.RESET_LABELCOLOR, { index, color })
    },
    resetFontsize_rect({ commit, state }, { index, size }) {
      
        if (typeof size == 'string') { 
            size=parseInt(size)
        }
        commit(types.RESET_LABELFONT, { index, size })
    },
    ctrVRects({ commit, state }, data) {
        data.map(item => {
        ...
        setTimeout(() => {
            commit(types.SETZINDEXRECT, {type:'top',rectType:['qrcode']})
       },200)
    },
    addRects({ commit, state }, { data, text, page,sha256,obj,enum_}) {
    ...
    delete_rect({ commit, state }, { key,page }) {
        commit(types.DELETE_RECT, key);
        
    },
     //  多个激活
     setActiveMultiple({ commit, state }, { id, page }) {
        commit(types.ENABLE_ACTIVE, id);
    },
    //单个激活
    setActive({ commit, state }, { id, page }) {
        // 
