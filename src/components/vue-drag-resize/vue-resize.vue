 ...
 *************************************************************************-->
 <template>
  <div :style="style" :class="active"  @touchstart="bodyDown($event)" @touchend="up($event)">
      <slot></slot>
  ...
  export default {
      ...
      methods: {
          ...
          deselect(e) {
            ...
            if (this.$parent.active != 'pageindex') { //shawn
                ...
                if(!!e.target.getAttribute('datakey')){
                  this.$emit('setActiveModuleStatus', e, e.target.getAttribute('datakey'), true)
                }  
                if(!!e.target.parentElement.getAttribute('datakey') ){
                  this.$emit('setActiveModuleStatus', e, e.target.parentElement.getAttribute('datakey'), true)
                }             
          ...
          stickMove(ev) {
              ...
              let newLeft = stickStartPos.left - delta.x;
              let newRight = stickStartPos.right + delta.x;
              switch (this.currentStick[0]) {//算出top值
                  case 'b':
              ...
              switch (this.currentStick[1]) {//算出left值
                  case 'r':
                      if (this.snapToGrid) {
