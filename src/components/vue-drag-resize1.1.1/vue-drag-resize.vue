...
<template>
    <div class="vdr" :style="style" :class="active || isActive ? 'active' : 'inactive'" @mousedown="bodyDown($event)"
         @touchstart="bodyDown($event)" @touchend="up($event)">
        <slot></slot>
    ...
    export default {
        ...
        methods: {
            ...
            async getActiveAll(nodes) {
                const activeAll =this.isActive? [{height:this.h,width:this.w,left:this.x,top:this.y}]:[];
                const XArray = []
                const YArray = []
                let groupWidth = 0
                let groupHeight = 0
            ...
            async snapCheck() {
                ...
                if (this.snap) {
                    // console.log('初始化当前组件的上下左右', 'activeLeft ='+ this.rawLeft,'activeRight ='+ this.rawLeft + "width,activeTop ="+ this.rawTop,'activeBottom ='+ this.rawTop + height )
                    ...
                    // 获取当前父节点下所有子节点
                    const nodes = this.$parent.rects.filter(item=>!item.active)//this.$el.parentNode.childNodes
                    ...
                    // item 和激活组件active的对应关系
                            ...
                            // console.log('ts',ts,Math.abs(t - activeBottom)+'-'+(t),activeBottom,item);
                            
                            const TS = Math.abs(b - activeBottom) < this.snapTolerance // item下边框和active的下边框
                            // console.log('TS',TS,Math.abs(b - activeBottom)+'-'+(b),activeBottom,item);
                        ...
                        //   console.log('x水平','hc-水平'+hc,'ts上下-'+ts,'TS下下-'+TS,'bs上上-'+bs,'BS下上-'+BS)
                        //   console.log('y垂直','vc-垂直'+vc,'ls左右-'+ls,'LS右右-'+LS,'rs-左左'+rs,'RS右左-'+RS)
                            if (ts) {
                        ...
                        //    console.log(tem.value)
                            ...
                            for (let i = 0; i <= arrTem.length; i++) {
                                ...
                                if (tem.display[i]) {
                                    ...
                                    refLine[horv][arrTem[i]].display = true//tem.display[i]
            ...
            hasnoactive(el, index) {
                // console.log(el.getAttribute('data') ,el.className.indexOf('noactive'),el);
                if (el.getAttribute('data') == 'noactive' || el.className.indexOf('noactive') >= 0) {
            ...
            deselect(e) {
                ...
                // console.log(haveno_,'haveno');
                
                if (this.$parent.active != 'pageindex') { //shawn
            ...
            bodyDown: function (ev) {
                ...
                if (!this.preventActiveBehavior) {
                    if (this.$parent.active != 'pageindex') {//shawn
                        this.$emit('setActiveModuleStatus', ev, this.parentKey_mo, true)
                    }
                    this.active = true;
            ...
            calcResizeLimitation() {
                ...
                const parentLim = this.parentLimitation ? !!this.parentKey_mo ? 1 : 0 : null;
