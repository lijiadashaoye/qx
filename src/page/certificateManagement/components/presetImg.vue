 ...
 *************************************************************************-->
 <template>
  <div class="imgupload">
    <img v-if="imgsrc.length>0" :src="imgsrc+'?'+new Date().getTime()" alt="" width="100%"  @dblclick="clickupload" height="100%" id="prevView" />

    <!-- <i class="el-icon-plus avatar-uploader-icon upload_" style="margin-top: 25%;transform: translateY(-100%); ">点击上传</i> -->
    <!--  -->
    <!-- <div   class="upload_"> -->
      <img v-if="imgsrc.length<=0" src="~assets/img/userUploadImg_.png" @ @dblclick="clickupload" title="双击上传" alt="" width="100%" height="100%" class="" />
  ...
  export default {
    ...
    methods: {
      ...
      prev(event) {
        ...
        // .then(result=>{
        //   ossUpload({ file: file[0], type: 1 }).then(response => {

        //     if (response.length > 0) {
        //       debugger
        //       this.$emit('setPresetImg', this.comontTmp, response,result.data)
        //     }
