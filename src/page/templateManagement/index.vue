  ...
  <div id="template">
    <div class="serchCondition">
      ...
      <div
        class="addBtn"
        v-if="
          ($parent.permissionArr.indexOf(100203) >= 0 &&
            this.$parent.userinfo.templateLimit -
              this.$parent.userinfo.templateNum >
              0) ||
          $parent.userinfo.isPlatformAdmin
        "
        @click="toPage('templateAdd')"
      >
        新建
      </div>
      <div
    ...
    <div class="main">
      <table cellspacing="0">
          ...
          <tr v-for="(item, index) in userList" :key="index">
            ...
            <td class="lastTd">
              ...
              <span
                class="option"
                v-if="
                  item.user.id == $parent.userinfo.id &&
                  $parent.permissionArr.indexOf(100201) >= 0 &&
                ...
                v-if="
                  ...
                  (item.templateSerial.length == 0 ||
                    (item.versionHistory.length > 0 &&
                      item.templateSerial == item.versionHistory[0]))
                "
                @click="toPage('templateEdit', item)"
                >编辑
              </span>
              ...
              <span
                class="option"
                v-if="
                  $parent.permissionArr.indexOf(100202) >= 0 &&
                  item.user.id == $parent.userinfo.id &&
                  item.status != 1 &&
                  (item.templateSerial.length == 0 ||
                    (item.versionHistory.length > 0 &&
              ...
              <span
                class="option"
                v-if="
                  $parent.permissionArr.indexOf(100206) &&
                  item.templateSerial.length > 0 &&
                  item.versionHistory.length > 0 &&
                  item.templateSerial != item.versionHistory[0]
                "
              ...
              <span
                v-if="
                  $parent.permissionArr.indexOf(100206) >= 0 &&
                  (item.templateSerial.length == 0 ||
                    (item.versionHistory.length > 0 &&
                      item.templateSerial == item.versionHistory[0]))
                "
        ...
        <nodata class="nodata" v-if="!userList || userList.length == 0">
          <div slot="header" class="title">暂无模板数据</div>
        </nodata>
      </table>
...
export default {
  ...
  methods: {
    async activeTem(item) {
      ...
      let params = {};
      let result = await certTemplateUpdateStatus({
        id: item.id,
