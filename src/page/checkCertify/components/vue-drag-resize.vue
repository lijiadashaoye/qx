...
export default {
  ...
  methods: {
    ...
    calcDragLimitation() {
      ...
      return {
        ...
        minTop: !!this.parentKey_mo ? 50 : 0,
        maxTop: !!this.parentKey_mo
          ? parentHeight - this.height - 50
    ...
    bodyMove(ev) {
      ...
      this.rawTop = Math.round(newTop / 10) * 10;
      this.rawBottom = newBottom;
      this.rawLeft = Math.round(newLeft / 10) * 10;
      this.rawRight = newRight;
      console.log("left-top", newLeft, newTop);
    ...
    calcResizeLimitation() {
      ...
      const parentLim = this.parentLimitation
        ? !!this.parentKey_mo
          ? 1
          : 0
        : null;
    ...
    stickMove(ev) {
      ...
      // console.log(stickStartPos,delta,pageX,pageY,'---stickMove')
      let newTop = stickStartPos.top - delta.y;
      let newBottom = stickStartPos.bottom + delta.y;
      let newLeft = stickStartPos.left - delta.x;
      let newRight = stickStartPos.right + delta.x;
      // console.log(this.currentStick)
      console.log(
        stickStartPos.right,
        delta.x,
        "---",
        stickStartPos.mouseX,
        pageX,
        this.parentScaleX
      );
      switch (this.currentStick[0]) {
      ...
      switch (this.currentStick[1]) {
        case "r":
          // console.log(this.snapToGrid,'this.snapToGrid',this.gridX,this.parentWidth)
          if (this.snapToGrid) {
            newRight =
              this.parentWidth -
  ...
  computed: {
    style() {
      return {
        top: this.top / 100 + "rem",
        left: this.left / 100 + "rem",
    ...
    vdrStick() {
      return (stick) => {
        const stickStyle = {
          width: `${stickSize / this.parentScaleX / 100}rem`,
          height: `${stickSize / this.parentScaleY / 100}rem`,
        };
        stickStyle[styleMapping.y[stick[0]]] = `${
          stickSize / this.parentScaleX / -2 / 100
        }rem`;
    ...
    height() {
      // console.log(this.parentHeight, this.top, this.bottom, this.parentHeight - this.top - this.bottom, '---------')
      return this.parentHeight - this.top - this.bottom;
    },

    rect() {
      // console.log('left:', Math.round(this.left),
  ...
  watch: {
    ...
    parentH(val) {
      // console.log(val, this.height, this.top, 'watchparentH')
